...
export function useJeopardySession() {
  ...
  async function flushRemoteState(nextSessionValue, nextTeamsValue) {
    if (!supabase || !remoteReadyRef.current) {
      return;
    }
  ...
  }, [hasLoadedSession, session, shouldUseLocalStorage]);
  ...
  }, [hasLoadedTeams, teams, shouldUseLocalStorage]);
  ...
  function resetGame() {
    ...
    void flushRemoteState(nextState.session, nextState.teams);
  }
